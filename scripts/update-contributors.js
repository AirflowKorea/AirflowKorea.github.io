...
const CONTRIBUTORS_FILE = path.join(__dirname, '../public/data/contributors.yaml');
...
async function fetchUserContributions(username, githubToken) {
  try {
    ...
    if (!response.ok) {
      console.error(`Failed to fetch data for ${username}: ${response.status} ${response.statusText}`);
      return null;
    }
...
function saveContributorsData(approvedContributors, contributorsData) {
  try {
    const data = {
      ...
      contributors: contributorsData.filter(contributor => contributor !== null)
    };
...
export { fetchUserContributions, loadApprovedContributors, saveContributorsData };